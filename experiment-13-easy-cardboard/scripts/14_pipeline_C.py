"""
Pipeline C: corrige o bug de naming do B.

Insight: o shader EC tem socket 'UV Name' = 'UVMap'. Se a UV ativa nao se
chama 'UVMap', o shader nao consegue ler a UV direcional do plugin (cai em
fallback).

Solucao:
  - Mantem a UV original do plugin com nome 'UVMap' (a que o shader LE)
  - Cria 'UV_BakeTarget' como UV nova com smart_project (a que o bake
    ESCREVE — active_render durante bake)
  - Apos bake, troca: UV_BakeTarget vira 'UVMap' (pro principled BSDF
    consumir) e a antiga vira 'UV_EC_legacy' ou eh removida

Uso:
  blender --background --python 14_pipeline_C.py -- <preset_name>
"""
...
def log(m): print(f"[C/{PRESET_NAME}] {m}", flush=True)
...
# CRITICAL: a UV original DEVE continuar se chamando "UVMap" pro shader EC ler
assert obj.data.uv_layers[0].name == "UVMap", f"UV inicial nao eh UVMap: {obj.data.uv_layers[0].name}"
log("UVMap original preservada com nome certo")

# Cria 2a UV pro bake target (smart_project pra ficar limpa em [0,1])
uv_bake = obj.data.uv_layers.new(name="UV_BakeTarget")
uv_bake.active = True
uv_bake.active_render = True
...
us = [d.uv[0] for d in uv_bake.data]
vs = [d.uv[1] for d in uv_bake.data]
log(f"UV_BakeTarget smart_project range: u=[{min(us):.3f},{max(us):.3f}] v=[{min(vs):.3f},{max(vs):.3f}]")

# CRITICAL: durante bake, UV ativa = UVMap (que o shader EC le)
# UV active_render = UV_BakeTarget (onde Cycles bake escreve)
obj.data.uv_layers["UVMap"].active = True
obj.data.uv_layers["UV_BakeTarget"].active_render = True
log(f"PRE-BAKE: active={obj.data.uv_layers.active.name}, active_render="+
    [l.name for l in obj.data.uv_layers if l.active_render][0])
...
# === Swap === agora UV_BakeTarget vira UVMap (pro principled ler ela)
# e a antiga vira algo descartavel
obj.data.uv_layers["UVMap"].name = "UV_EC_legacy"
obj.data.uv_layers["UV_BakeTarget"].name = "UVMap"
obj.data.uv_layers["UVMap"].active = True
...
# Remove UV legacy + vertex colors pra GLB ficar limpo
obj.data.uv_layers.remove(obj.data.uv_layers["UV_EC_legacy"])
...
log("=== DONE ===")
print(f"[C/{PRESET_NAME}] === SUCCESS ===", flush=True)
